...
def generate_fig2_sheaf():
    ...
    plt.tight_layout()
    plt.savefig('figure_2_sheaf.png', dpi=300)
    plt.close()

def generate_fig3_arch():
    ...
    plt.tight_layout()
    plt.savefig('figure_3_arch.png', dpi=300)
    plt.close()

def generate_fig4_dynamics():
    ...
    ax2 = ax1.twinx()  
    color = 'tab:orange'
    ax2.set_ylabel('Curvature $\\sigma$', color=color)  
    ax2.plot(steps, sigma, color=color, linestyle='--', label='Sigma')
    ax2.tick_params(axis='y', labelcolor=color)
    ax2.set_ylim(0, 3.0)

    plt.title("Figure 4: Training Dynamics (Loss vs Curvature)")
    plt.tight_layout()
    plt.savefig('figure_4_dynamics.png', dpi=300)
    plt.close()

def generate_fig5_affinity():
    ...
    plt.tight_layout()
    plt.savefig('figure_5_affinity.png', dpi=300)
    plt.close()

def generate_fig7_svd():
    ...
    plt.tight_layout()
    plt.savefig('figure_7_svd.png', dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    generate_fig2_sheaf()
    generate_fig3_arch()
    generate_fig4_dynamics()
    generate_fig5_affinity()
    generate_fig7_svd()
    print("Done.")
