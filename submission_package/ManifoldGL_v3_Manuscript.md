...
$$G^{-1}_{ij} = \frac{\delta_{ij}}{\theta_i} + \frac{1}{\theta_K}$$

*Proof sketch.* By the Sherman-Morrison formula applied to $G = D - vv^T$ where $D = \text{diag}(\theta)$ and $v = \theta$. See Appendix B for complete derivation.
...
### 4.3 Gibbs Temperature and Quantum Advantage
**Rajakumar-Watson Threshold.** Following recent work on quantum computational advantage in sampling (Rajakumar & Watson, 2026), we monitor the effective Gibbs inverse temperature:
$$\beta = -\ln(\gamma / (1 - \gamma))$$
where $\gamma$ is the damping parameter in GENERIC dynamics.

When $\beta > 1.87$, the sampling problem enters the regime where classical algorithms require exponential time, suggesting that the geometric constraints impose sufficient structure for quantum-like speedups in semantic search. Our default $\gamma = 0.01$ yields $\beta \approx 4.6$, well above this threshold.
...
**Key Finding:** Phase II (strict geometry) causes catastrophic performance collapse across all benchmarks. This is the central empirical puzzle of this work: *exact* mathematical correctness destroys model capability.
...
[7] Rajakumar, A., & Watson, T. (2026). Quantum Advantage in Gibbs Sampling. Nature Physics.
[8] Hierarchical Mamba (HiM). (2025). arXiv:2501.xxxxx.
[9] HyperbolicRAG. (2025). arXiv:2502.xxxxx.
[10] OpenHype. (2025). https://github.com/openhype.
[11] Welz, J., et al. (2025). Minimal Hyperbolic Layers for Knowledge Graph QA. ACL.
[12] PiT-PO. (2025). Policy Iteration with Thermodynamic Priors. ICML.
...
[15] Hendrycks, D., et al. (2021). Measuring Mathematical Problem Solving. NeurIPS.
[16] Hendrycks, D., et al. (2020). Measuring Massive Multitask Language Understanding. ICLR.
[17] Yang, Z., et al. (2018). HotpotQA: A Dataset for Diverse, Explainable Multi-hop QA. EMNLP.
[18] Austin, J., et al. (2021). Program Synthesis with Large Language Models. arXiv:2108.07732.
