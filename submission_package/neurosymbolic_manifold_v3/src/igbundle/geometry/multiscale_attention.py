...
class MultiScaleMetric(nn.Module):
    ...
    def _compute_scale_attention(self, positions: torch.Tensor) -> torch.Tensor:
        ...
        # Ensure scale features match model dtype (torch.var/norm can be float32)
        if scale_features.dtype != positions.dtype:
            scale_features = scale_features.to(positions.dtype)
        ...
        if scale_weights.dtype != positions.dtype:
            scale_weights = scale_weights.to(positions.dtype)
...
class CrossScaleAttention(nn.Module):
    ...
    def forward(self, positions: torch.Tensor, multiscale_metrics: List[RiemannianMetric]) -> torch.Tensor:
        ...
        for scale_idx, metric in enumerate(multiscale_metrics):
            ...
            scale_id = torch.full((B, T, P, 1), scale_idx / self.num_scales, 
        ...
        if fused_output.dtype != positions.dtype:
            fused_output = fused_output.to(positions.dtype)
...
class MultiScaleParallelTransport(nn.Module):
    ...
    def forward(self, vectors: torch.Tensor, paths: torch.Tensor,
        ...
        scale_weights = torch.softmax(
            torch.randn(self.num_scales, device=vectors.device, dtype=vectors.dtype), dim=0
        )
        ...
        if final_transport.dtype != vectors.dtype:
            final_transport = final_transport.to(vectors.dtype)
