...
**Riemannian Metric.** The adapter learns a metric tensor g on M approximated via the Fisher information matrix of the fiber distributions. Curvature is regularized toward κ = -1 via a conformal scalar-curvature estimator; note that with the default (fixed) conformal factor this regularizer cannot bind — see `config.learnable_conformal` and `draft_paper_falsification.md` §2.6.
...
The fixed geometric kernels produce measurable, non-trivial structure. **Scope caveat (2026-07 audit):** these tests verify the *fixed* Poincaré operations and the hardcoded conformal factor — not learned geometry. In particular, the curvature reading below is weight-invariant with the default configuration (a constant of the architecture evaluated at the coordinate-norm distribution; see `draft_paper_falsification.md` §2.6) and must not be read as a training outcome.

| Metric | Value | Interpretation |
|:---|:---:|:---|
| Curvature K | -5.63 | Architecture constant of the hardcoded conformal factor at the projection boundary (r ≈ 0.95); NOT learned hyperbolicity. The regularizer target (-1.0) is unreachable with default config — enable `learnable_conformal` for it to bind |
...
| Faithfulness | **6/6** | All geometric verification tests pass (fixed kernels) |
